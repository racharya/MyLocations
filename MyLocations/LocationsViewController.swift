...
class LocationsViewController: UITableViewController {
    ...
    //gets called once for each section in table view
    //here we create a label for section name that functions as a separator line
    override func tableView(tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        let labelRect = CGRect(x: 15, y: tableView.sectionHeaderHeight - 14, width: 300, height: 14)
        let label = UILabel(frame: labelRect)
        label.font = UIFont.boldSystemFontOfSize(11)
        
        //titleForHeaderInSection is an optional method so need to unwarap it using tableView! up front
